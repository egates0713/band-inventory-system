              ...
              {importResults.type === 'csv' && (
                <div>
                  ...
                  <p className="text-sm text-slate-600 mb-2">
                    Found {(importResults.data as unknown[])?.length || 0} rows. This is a preview - actual import functionality would be implemented here.
                  </p>
                  <div className="bg-slate-50 p-2 rounded text-xs max-h-40 overflow-auto">
                    <pre>{JSON.stringify((importResults.data as unknown[])?.slice(0, 3), null, 2)}</pre>
                  </div>
                </div>
              )}
